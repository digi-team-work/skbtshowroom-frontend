...
export default function HomeClient({basePath, linkPath, video}) {
  ...
  return (
    <>
      {init && (
        <>
          {mq ? (<HomeDesktop basePath={basePath} linkPath={linkPath} video={video} />):(<HomeMobile basePath={basePath} linkPath={linkPath} />)}
        </>
      )}
    </>
  )
